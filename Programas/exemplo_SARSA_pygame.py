...
class SARSAEnvironment:
    """
    Classe que implementa um ambiente de grade para o algoritmo SARSA.
    """

    def __init__(self):
        """
        Inicializa a classe SARSAEnvironment com os parâmetros padrão do ambiente.
        """
    ...
    def sarsa(self):
        """
        Implementação do algoritmo SARSA para treinar o agente no ambiente.
        """
        ...
        for episode in range(2000):
            ...
            while state != self.goal_state:
                ...
                next_action = max(Q[next_state], key=Q[next_state].get) if random.uniform(0, 1) > epsilon else random.choice(self.actions)

                # Atualiza a função Q com base na equação SARSA
            ...
            if episode % 100 == 0:
                with open('sarsa_model.pkl', 'wb') as file:
                    pickle.dump(Q, file)
...
# Criar uma instância da classe e executar o SARSA
env = SARSAEnvironment()
env.sarsa()
